...
Sharding pattern (mirroring tpu-inference's ``run_jax_gdn_attention``):
the conv + recurrence pipeline runs inside :func:`jax.shard_map` with
explicit ``in_specs`` / ``out_specs``. Each device sees only its local
shard, so the kernels operate on per-shard head counts (``n_kq // TP``,
``n_v // TP``) without JAX sharding-inference edge cases. Returns
