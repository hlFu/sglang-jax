...
def jax_causal_conv1d_prefill(
    ...
    # served from initial state slot (K-1) + p' (newest at K-2).
    o = jnp.arange(K)
...
def ragged_gated_delta_rule_ref(
    ...
    *,
    ...
    Token-by-token ``jax.lax.scan`` over a packed ragged batch. Adapted
    from ``tpu_inference.layers.common.ragged_gated_delta_rule_ref``;
    simplified to consume sglang-jax's ``cu_seqlens`` directly (no
    separate ``distribution`` array — ``cu_seqlens[-1]`` already gives the
    valid-token count).
    ...
    key = mixed_qkv[..., key_dim : 2 * key_dim]
    value = mixed_qkv[..., 2 * key_dim :]
    ...
    def scan_fn(state_buf, xs):
        ...
        q_t, k_t, v_t, b_t, a_t, req_idx, is_valid = xs

        state = state_buf[req_idx]  # [n_v, d_k, d_v]

        q_h = q_t.reshape(n_kq, d_k)
        k_h = k_t.reshape(n_kq, d_k)
        v_h = v_t.reshape(n_v, d_v)
        if repeat_factor > 1:
            q_h = jnp.repeat(q_h, repeat_factor, axis=0)
            k_h = jnp.repeat(k_h, repeat_factor, axis=0)
