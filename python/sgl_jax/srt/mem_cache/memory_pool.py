...
class HybridReqToTokenPool(ReqToTokenPool):
    """``ReqToTokenPool`` + ``RecurrentStatePool`` side-by-side.
    ...
    def alloc(self, need_size: int = 1) -> list[int] | None:
        ...
        if req_slots is None:
            return None
        mamba_slots = self.recurrent_state_pool.alloc(need_size)
    ...
    def free(self, free_index):
        ...
        self.recurrent_state_pool.free(mamba_slots)
    ...
    # --- pytree -------------------------------------------------------------
    def tree_flatten(self):
        children = (self.req_to_token, self.recurrent_state_pool)
        aux_data = {
            "size": self.size,
            "max_context_len": self.max_context_len,
            "dtype": self.dtype,
    ...
    def tree_unflatten(cls, aux_data, children):
        ...
        obj.req_to_token, obj.recurrent_state_pool = children
        return obj
